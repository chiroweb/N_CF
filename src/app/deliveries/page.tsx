  ...
  return (
    <div ref={pageRef} className="bg-paper min-h-screen">
      ...
      <div className="container-content pt-20 lg:pt-28">
        ...
        >
          <span className="caption-style text-ink/60 mr-auto">
            PRODUCT LINE
          </span>
          <ProductTabButton
            active={product === "afterburner"}
            label="AFTERBURNER"
            sub="ROASTER LEDGER · 2024—2025"
            onClick={() => setProduct("afterburner")}
          />
          <ProductTabButton
            active={product === "nutstar"}
            label="NUT BUTTER MACHINE"
            sub="NUTSTAR · 2025—2026"
      ...
      {product === "afterburner" ? (
        ...
        <>
          ...
          <section id="records" className="container-content pb-24">
            <div className="scroll-fade flex items-baseline justify-between border-t-2 border-ink pt-6 mb-10 opacity-0">
              <span className="caption-style text-ink/90">
                DELIVERED · {nutstarDelivered.length} CUSTOMERS
              </span>
              <span className="caption-style text-ink/80">
                {String(
            ...
            {nutstarPending.length > 0 && (
              <div className="mt-20">
                <div className="scroll-fade flex items-baseline justify-between border-t-2 border-ink pt-6 mb-10 opacity-0">
                  <span className="caption-style text-ink/90">
                    SCHEDULED · {nutstarPending.length} CUSTOMERS
                  </span>
                  <span className="caption-style text-ink/80">
                    {String(
                      nutstarPending.reduce((a, r) => a + r.qty, 0)
                    ).padStart(2, "0")}{" "}
                    UNITS · {NUTSTAR_SUMMARY.latest}
                  </span>
                </div>
                <div className="columns-1 md:columns-2 lg:columns-3 gap-x-10 lg:gap-x-14">
  ...
  return (
    <article className="break-inside-avoid pt-5 pb-6 border-t border-ink/20 mb-1">
      ...
      <div className="flex items-end justify-between gap-3 mb-3">
        <div className="flex flex-col gap-0.5 min-w-0">
          <span className="caption-style text-ink/90">
            {r.businessType.toUpperCase()}
          </span>
          {r.operatingMode && (
            <span className="caption-style text-ink/70 truncate max-w-[22ch]">
              MODE · {r.operatingMode}
            </span>
          )}
          <span className="caption-style text-ink/65">
            {r.deliveryDate}
          </span>
        </div>
